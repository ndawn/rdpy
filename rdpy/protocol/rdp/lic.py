...
from rdpy.core.type import CompositeType, CallableValue, UInt8, UInt16Le, UInt32Le, Bytes, sizeof, FactoryType, ArrayType, Stream
...
class LicenseBinaryBlob(CompositeType):
    ...
    def __init__(self, blobType = BinaryBlobType.BB_ANY_BLOB, optional = False):
        ...
        self.blobData = Bytes(readLen = self.wBlobLen)
...
class ProductInformation(CompositeType):
    ...
    def __init__(self):
        ...
        self.pbCompanyName = Bytes("Microsoft Corporation", readLen = self.cbCompanyName, unicode = True)
        self.cbProductId = UInt32Le(lambda:sizeof(self.pbProductId))
        #may contain "A02" from microsoft license server
        self.pbProductId = Bytes("A02", readLen = self.cbProductId, unicode = True)
...
class ServerLicenseRequest(CompositeType):
    ...
    def __init__(self, readLen = None):
        CompositeType.__init__(self, readLen = readLen)
        self.serverRandom = Bytes("\x00" * 32, readLen = CallableValue(32))
...
class ClientNewLicenseRequest(CompositeType):
    ...
    def __init__(self, readLen = None):
        ...
        self.clientRandom = Bytes("\x00" * 32, readLen = CallableValue(32))
...
class ServerPlatformChallenge(CompositeType):
    ...
    def __init__(self, readLen = None):
        ...
        self.MACData = Bytes(readLen = CallableValue(16))
...
class LicPacket(CompositeType):
    ...
    def __init__(self, message = None):
        ...
        def LicensingMessageFactory():
            ...
            return Bytes(readLen = self.wMsgSize - 4)
...
class LicenseManager(object):
    ...
    def sendClientChallengeResponse(self, platformChallenge):
        ...
        #generate hwid
        s = Stream()
        s.writeType((UInt32Le(2), Bytes(self._hostname + self._username + "\x00" * 16)))
        hwid = s.getvalue()[:20]
