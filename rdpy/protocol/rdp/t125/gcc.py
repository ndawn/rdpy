...
import hashlib
from rdpy.core.type import UInt8, UInt16Le, UInt32Le, CompositeType, CallableValue, Bytes, Stream, sizeof, FactoryType, ArrayType
...
class DataBlock(CompositeType):
    ...
    def __init__(self, dataBlock = None):
        ...
        def DataBlockFactory():
            ...
            #read entire packet
            return Bytes(readLen = self.length - 4)
...
class ClientCoreData(CompositeType):
    ...
    def __init__(self, readLen = None):
        ...
        self.clientName = Bytes("rdpy" + "\x00"*11, readLen = CallableValue(32), unicode = True)
        self.keyboardType = UInt32Le(KeyboardType.IBM_101_102_KEYS)
        self.keyboardSubType = UInt32Le(0)
        self.keyboardFnKeys = UInt32Le(12)
        self.imeFileName = Bytes("\x00"*64, readLen = CallableValue(64), optional = True)
        ...
        self.clientDigProductId = Bytes("\x00"*64, readLen = CallableValue(64), optional = True)
...
class ServerSecurityData(CompositeType):
    ...
    def __init__(self, readLen = None):
        ...
        self.serverRandom = Bytes(readLen = self.serverRandomLen, conditional = lambda:not(self.encryptionMethod.value == 0 and self.encryptionLevel == 0))
...
class ProprietaryServerCertificate(CompositeType):
    ...
    def __init__(self):
        ...
        self.SignatureBlob = Bytes(readLen = CallableValue(lambda:(self.wSignatureBlobLen.value - sizeof(self.padding))))
        self.padding = Bytes(b"\x00" * 8, readLen = CallableValue(8))
...
class CertBlob(CompositeType):
    ...
    def __init__(self):
        ...
        self.abCert = Bytes(readLen = self.cbCert)
...
class X509CertificateChain(CompositeType):
    ...
    def __init__(self):
        ...
        self.padding = Bytes(readLen = CallableValue(lambda:(8 + 4 * self.NumCertBlobs.value)))
...
class RSAPublicKey(CompositeType):
    ...
    def __init__(self, readLen):
        ...
        self.modulus = Bytes(readLen = CallableValue(lambda:(self.keylen.value - 8)))
...
class ChannelDef(CompositeType):
    ...
    def __init__(self, name = "", options = 0):
        CompositeType.__init__(self)
        #name of channel
        self.name = Bytes(name[0:8] + "\x00" * (8 - len(name)), readLen = CallableValue(8))
        #unknown
