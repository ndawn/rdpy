...
from rdpy.core.type import CompositeType, FactoryType, UInt8, UInt16Le, UInt32Le, Bytes, sizeof, Stream
...
class Event(CompositeType):
    ...
    def __init__(self, event = None):
        ...
        def EventFactory():
            ...
            #read entire packet
            return Bytes(readLen = self.length)
...
class UpdateEvent(CompositeType):
    ...
    def __init__(self, readLen = None):
        ...
        self.data = Bytes(readLen = self.length)
...
class InfoEvent(CompositeType):
    ...
    def __init__(self, readLen = None):
        ...
        self.username = Bytes(readLen = self.lenUsername)
        self.lenPassword = UInt16Le(lambda:sizeof(self.password))
        self.password = Bytes(readLen = self.lenPassword)
        self.lenDomain = UInt16Le(lambda:sizeof(self.domain))
        self.domain = Bytes(readLen = self.lenDomain)
        self.lenHostname = UInt16Le(lambda:sizeof(self.hostname))
        self.hostname = Bytes(readLen = self.lenHostname)
